...
import { GET_ALL_NEWS_PAGINATED, GET_NEWS, CREATE_NEWS, UPDATE_NEWS, DELETE_NEWS } from '../constants/types';
...
function* getNews({ payload }) {
  ...
  try {
    const { data: {data} } = yield call(newsApi.getNews, payload);
...
function* getNewsPaginated({ payload }) {
    yield put(newsAction.setNewsLoading(true));
    try {
        const {data: {data}} = yield call(newsApi.getAllNewsPaginated, payload);
...
function* createNews({ payload }) {
  yield put(newsAction.setNewsLoading(true));
  try {
    const { data: {data} } = yield call(newsApi.createNews, payload);
...
function* updateNews({ payload }) {
  yield put(newsAction.setNewsLoading(true));
  try {
    const { data: {data} } = yield call(newsApi.updateNews, payload);
...
function* deleteNews({ payload }) {
  yield put(newsAction.setNewsLoading(true));
  try {
    const { data: {data} } = yield call(newsApi.deleteNews, payload);
